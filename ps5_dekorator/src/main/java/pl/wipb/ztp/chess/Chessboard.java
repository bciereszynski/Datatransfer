...
public class Chessboard extends JPanel {
	...
	Chessboard() {
		board.put(new Point(0, 2), new PlacementDecorator(new Piece(11, 0, 2)));
		board.put(new Point(0, 6), new PlacementDecorator(new Piece(0, 0, 6)));
		board.put(new Point(1, 4), new PlacementDecorator(new Piece(6, 1, 4)));
		board.put(new Point(1, 5), new PlacementDecorator(new Piece(5, 1, 5)));
		board.put(new Point(3, 7), new PlacementDecorator(new Piece(1, 3, 7)));
		board.put(new Point(4, 3), new PlacementDecorator(new Piece(6, 4, 3)));
		board.put(new Point(4, 4), new PlacementDecorator(new Piece(7, 4, 4)));
		board.put(new Point(5, 4), new PlacementDecorator(new Piece(6, 5, 4)));
		board.put(new Point(5, 6), new PlacementDecorator(new Piece(0, 5, 6)));
		board.put(new Point(6, 5), new PlacementDecorator(new Piece(0, 6, 5)));
		board.put(new Point(7, 4), new PlacementDecorator(new Piece(0, 7, 4)));
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent ev) {
				...
				tmp = dragged;
				dragged = new MovementDecorator(dragged);
				mouse = ev.getPoint();
			}
		...
		this.addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent ev) {
				dragged.moveTo((int)(ev.getX() - Chessboard.this.mouse.getX()), (int)(ev.getY() - Chessboard.this.mouse.getY()));
				repaint();
			}
		});
	}
